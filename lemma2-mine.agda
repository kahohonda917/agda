...
mutual
  ...
  c-type Nat = ⊤
  c-type Tbool = ⊤
  c-type (_⇒_⟨_⟩_⟨_⟩_ {μα} τ₂ τ₁ μ[β]α α μβ β) =
    {μk : trail} →
    c-type τ₂ →
    (c-type τ₁ → c-trail μα → compatible μk μα μα × c-type α) →
    c-trail μβ →
    compatible μk μβ μβ × c-type β

    -- {μk : trail} → (c : compatible μk μβ μβ) →
    --                 compatible μk μα μα
...
mutual
  ...
  c-value (Var x) = x
  c-value (Num n) = tt
  c-value (Fun e) = λ v k t → c-term (e v) k t

  c-term : {τ α β : typ} {μα μβ μk : trail} →
           {μ[β]α : trails[ μβ ] μα} →
           term[ c-type ] τ ⟨ μ[β]α ⟩ α ⟨ μβ ⟩ β →
           (c-type τ → c-trail μα →
            compatible μk μα μα × c-type α) →
           c-trail μβ →
           compatible μk μβ μβ × c-type β
  c-term (Val v) k t = k (c-value v) t
  c-term (App e₁ e₂) k t =
    c-term e₁ (λ v₁ t₁ → c-term e₂ (λ v₂ t₂ → v₁ v₂ k t₂) t₁) t
  c-term (Plus e₁ e₂) k t =
    c-term e₁ (λ v₁ t₁ → c-term e₂ (λ v₂ t₂ → k tt t₂) t₁) t
  c-term (Control id c₁ c₂ e) k t = {!!}
  c-term (Prompt {μᵢ = ∙} refl e) k t =
    k (proj₂ (c-term {μk = ∙} e (λ v t' → refl , v) tt)) t
  c-term (Prompt {μᵢ = τ ⇒ τ' , .∙} (refl , refl , refl) e) k t =
    k (proj₂ (c-term {μk = ∙} e (λ v t' → refl , t' v tt) tt)) t

h : ∀ {var : cpstyp → Set} {μα μβ μk μ₀ : trail}
       {μ[β]α : trails[ μβ ] μα}
...
l' {μβ = τ ⇒ τ' , ∙} (refl , refl , refl) c₂ = refl , refl , refl
l' {τ₂ ⇒ τ''' , ∙} {μβ = .τ₂ ⇒ .τ''' , (τ₁ ⇒ τ'' , μβ)} (refl , refl , refl , refl , refl) c₂ = refl , refl , refl
l' {τ₂ ⇒ τ''' , (τ ⇒ τ' , μα)} {μβ = .τ₂ ⇒ .τ''' , (.τ ⇒ .τ' , .μα)} {∙} (refl , refl , refl , refl , refl , refl , c₁) refl = refl , refl , refl , refl , refl , refl , c₁
l' {τ₂ ⇒ τ''' , (τ ⇒ τ' , μα)} {μβ = .τ₂ ⇒ .τ''' , (τ₁ ⇒ τ'' , (.τ ⇒ .τ' , μβ))} {.τ₁ ⇒ .τ'' , (.τ ⇒ .τ' , μ₀)} (refl , refl , refl , refl , refl , refl , refl , refl , c₁) (refl , refl , refl , refl , refl , refl , c₂) = refl , refl , refl , refl , refl , refl , l' (refl , refl , c₁) c₂
-- {!l' (refl , refl , c₁) c₂!}
  -- with l' c₁ {!!}
  -- with l' (refl , refl , c₁) {!!}

{-
l :  {μk μα μβ μ₀ μ₁ : trail}
     {τ₁ τ₂ : typ}
     (c₁ : compatible (τ₁ ⇒ τ₂ , μk) μβ μβ)
     (c₂ : compatible μβ μ₀ μα)
     (c₃ : compatible μβ μ₁ μk)
     (k : ⟦ τ₁ ⇒ τ₂ , μk ⟧μ)
     (t : ⟦ μβ ⟧μ)
     (k't' : ⟦ μ₀ ⟧μ) →
     Σ-syntax (compatible (τ₁ ⇒ τ₂ , μk) μα μα)
     (λ c' →
       append-trail c₂ (cons-trail c₁ k t) k't' ≡
       cons-trail c' k (append-trail c₂ t k't'))
l {μβ = τ ⇒ τ' , μβ} {∙} (refl , refl , c₁) refl c₃ k t tt = (refl , refl , c₁) , refl
l {μα = τ₂ ⇒ τ''' , ∙} {μβ = .τ₂ ⇒ .τ''' , ∙} {τ₁ ⇒ τ'' , μ₀} (refl , refl , refl) (refl , refl , ()) c₃ k t k't'
l {μα = τ₂ ⇒ τ''' , (τ ⇒ τ' , μα)} {μβ = .τ₂ ⇒ .τ''' , ∙} {τ₁ ⇒ τ'' , μ₀} (refl , refl , refl) (refl , refl , ()) c₃ k t k't'
l {τ₃ ⇒ τ'''' , ∙} {μα = .τ₃ ⇒ .τ'''' , ∙} {μβ = .τ₃ ⇒ .τ'''' , (τ ⇒ τ' , μβ)} {.τ ⇒ .τ' , .μβ} (refl , refl , refl , refl , refl) (refl , refl , refl) c₃ k t k't' = (refl , refl , refl) , refl
l {τ₃ ⇒ τ'''' , ∙} {μα = .τ₃ ⇒ .τ'''' , (τ₂ ⇒ τ''' , μα)} {μβ = .τ₃ ⇒ .τ'''' , (τ ⇒ τ' , μβ)} {.τ ⇒ .τ' , μ₀} (refl , refl , refl , refl , refl) (refl , refl , refl , refl , c₂) c₃ k t k't' = (refl , refl , refl , refl , refl) , refl
l {τ₃ ⇒ τ'''' , (τ₂ ⇒ τ''' , μk)} {μα = .τ₃ ⇒ .τ'''' , ∙} {μβ = .τ₃ ⇒ .τ'''' , (τ ⇒ τ' , μβ)} {.τ ⇒ .τ' , .μβ} {μ₁} (refl , refl , refl , refl , refl , refl , c₁) (refl , refl , refl) c₃ k t k't' = {!μ₁!}
l {τ₃ ⇒ τ'''' , (τ₂ ⇒ τ''' , μk)} {μα = .τ₃ ⇒ .τ'''' , (τ₄ ⇒ τ''''' , μα)} {μβ = .τ₃ ⇒ .τ'''' , (τ ⇒ τ' , μβ)} {τ₁ ⇒ τ'' , μ₀} (refl , refl , refl , refl , refl , refl , c₁) (refl , refl , c₂) c₃ k t k't' = {!!}

{-
l {∙} {τ ⇒ τ' , ∙} (refl , refl , ()) c₂ k t k't'
l {∙} {τ ⇒ τ' , (τ₁ ⇒ τ'' , μα)} (refl , refl , ()) c₂ k t k't'
l {τ ⇒ τ' , .∙} {.τ ⇒ .τ' , ∙} {∙} {μγ = ∙} (refl , refl , refl) () k t k't'
l {τ ⇒ τ' , .∙} {.τ ⇒ .τ' , ∙} {τ₁ ⇒ τ'' , μβ} {μγ = ∙} (refl , refl , refl) () k t k't'
l {τ ⇒ τ' , .∙} {.τ ⇒ .τ' , ∙} {∙} {μγ = .τ ⇒ .τ' , ∙} (refl , refl , refl) refl k t tt = (refl , refl , refl) , refl
l {τ ⇒ τ' , .∙} {.τ ⇒ .τ' , ∙} {∙} {μγ = τ₁ ⇒ τ'' , (τ₂ ⇒ τ''' , μγ)} (refl , refl , refl) () k t tt
l {τ ⇒ τ' , .∙} {.τ ⇒ .τ' , ∙} {τ₃ ⇒ τ'''' , μβ} {μγ = τ₁ ⇒ τ'' , (τ₂ ⇒ τ''' , μγ)} (refl , refl , refl) () k t k't'
l {τ ⇒ τ' , ∙} {.τ ⇒ .τ' , (τ₂ ⇒ τ''' , μα)} {∙} (refl , refl , refl , refl , refl) refl k t tt = (refl , (refl , (refl , refl , refl))) , refl
l {τ ⇒ τ' , ∙} {.τ ⇒ .τ' , (τ₂ ⇒ τ''' , μα)} {.τ₂ ⇒ .τ''' , .μα} {.τ ⇒ .τ' , ∙} (refl , refl , refl , refl , refl) (refl , refl , refl) k t k't' = (refl , refl , refl) , refl
l {τ ⇒ τ' , ∙} {.τ ⇒ .τ' , (τ₂ ⇒ τ''' , μα)} {.τ₂ ⇒ .τ''' , μβ} {.τ ⇒ .τ' , (τ₃ ⇒ τ'''' , μγ)} (refl , refl , refl , refl , refl) (refl , refl , refl , refl , c₂) k t k't' = (refl , refl , refl , refl , refl) , refl
l {τ ⇒ τ' , (τ₁ ⇒ τ'' , μ)} {.τ ⇒ .τ' , (τ₂ ⇒ τ''' , μα)} {∙} (refl , refl , refl , refl , refl , refl , c) refl k t tt = (refl , refl , refl , refl , refl , refl , c) , refl
l {τ ⇒ τ' , (τ₁ ⇒ τ'' , μ)} {.τ ⇒ .τ' , (τ₂ ⇒ τ''' , μα)} {.τ₂ ⇒ .τ''' , .μα} {.τ ⇒ .τ' , ∙} (refl , refl , refl , refl , refl , refl , c) (refl , refl , refl) k t k't' = {!cons-trail (refl , refl , refl , refl , ?) k (append-trail (refl , refl , refl) t k't')!}
l {τ ⇒ τ' , (τ₁ ⇒ τ'' , μ)} {.τ ⇒ .τ' , (τ₂ ⇒ τ''' , μα)} {.τ₂ ⇒ .τ''' , μβ} {.τ ⇒ .τ' , (τ₄ ⇒ τ''''' , μγ)} (refl , refl , refl , refl , refl , refl , c) (refl , refl , refl , refl , c₂) k t k't' = {!!}
-}

{-
l {μα = τ ⇒ τ' , μα} {∙} (refl , refl , c) refl k t tt = (refl , refl , c) , refl
l {μ = ∙} {μα = τ ⇒ τ' , .(τ₁ ⇒ τ'' , μβ)} {τ₁ ⇒ τ'' , μβ} {.τ ⇒ .τ' , ∙} (refl , refl , ()) (refl , refl , refl) k t k't'
l {μ = τ₂ ⇒ τ''' , ∙} {μα = .τ₂ ⇒ .τ''' , .(τ₁ ⇒ τ'' , μβ)} {τ₁ ⇒ τ'' , μβ} {.τ₂ ⇒ .τ''' , ∙} (refl , refl , refl , refl , refl) (refl , refl , refl) k t k't' = {!!}
l {μ = τ₂ ⇒ τ''' , (τ ⇒ τ' , μ)} {μα = .τ₂ ⇒ .τ''' , .(τ₁ ⇒ τ'' , μβ)} {τ₁ ⇒ τ'' , μβ} {.τ₂ ⇒ .τ''' , ∙} (refl , refl , refl , refl , c) (refl , refl , refl) k t k't' = {!!}
l {μ = τ₁ ⇒ τ'' , μ} {μα = .τ₁ ⇒ .τ'' , (τ₃ ⇒ τ'''' , μα)} {.τ₃ ⇒ .τ'''' , μβ} {.τ₁ ⇒ .τ'' , (τ₂ ⇒ τ''' , μγ)} (refl , refl , refl , refl , c) (refl , refl , refl , refl , c₂) k t k't' = {!!}
-}
...
aux₄ : ∀ {var : cpstyp → Set}{τ}{α}{β}{μα}{μβ}{μk}
         ...
         (cpsreduce {var}
          (cpsTerm e (λ v t' → κ v t') (CPSCons c (CPSVar k) t))
          (cpsTerm e (λ v t' → κ v (CPSCons c' (CPSVar k) t')) t))
aux₄ = {!!}
...
aux₄ (App e₁ e₂) {c} κ k t = {! !}
{-
  with aux₄ e₁ {c} (λ v₁ t₁ → cpsTerm e₂ (λ v₂ t₂ →
    CPSApp (CPSApp (CPSApp (CPSVal v₁) (CPSVal v₂)) (CPSVal (CPSFun (λ v → CPSVal (CPSFun (λ t'' → κ (CPSVar v) (CPSVar t'')))))))
           (CPSVal t₂)
  ) t₁) k t
... | (c' , red) = {!!} , (begin
      (cpsTerm e₁
              ...
              (CPSVal
               (CPSFun
                (λ v →
                   CPSVal
                   (CPSFun
                    (λ t'' → κ (CPSVar v) (CPSCons {!!} (CPSVar k) (CPSVar t''))))))))
             (CPSVal t₂)))
       t)
  ∎)
-}
aux₄ (Plus e₁ e₂) {c} κ k t = {!!}
aux₄ (Control id₁ c₁ c₂ e) κ k t = {!h k t!}
...
correctEta k t sch sch' (RBeta x) = {!!}
correctEta k t sch sch' RPlus = {!!}
correctEta k t sch sch' (RFrame f red) = {!!}
correctEta k t sch sch' RPrompt = {!!}
...
correctEta {var} {τ₁} {α} {.α} {μα} {.μα} {μs}
  ...
  ⟶⟨ {!!} ⟩
  {-
  -- ⟶⟨ rLet₁ (rLet₁ (rFun (λ x₁ → rFun (λ x₂ → rFun (λ x₃ → proj₂ (aux₄ (pcontext-plug p₂ (Val (Var x₁))) {{!!}} (CPSIdk id₀) x₂ (CPSVar x₃)) ))))) ⟩
  CPSLet
    (CPSLet
     (CPSVal
      (CPSFun
          ...
          (CPSFun
           (λ z₁ →
              ...
              (CPSFun
               (λ z₂ →
                  cpsTerm (pcontext-plug p₂ (Val (Var z)))
                  (λ v t'' → CPSIdk id₀ v (CPSCons (proj₁ (aux₄ {var} (pcontext-plug p₂ (Val (Var z))) {c = c₁} (CPSIdk id₀) z₁ (CPSVar z₂))) (CPSVar z₁) t''))
                  (CPSVar z₂))))))))
     (λ x' → cpsTerm (e x') (CPSIdk id) CPSId))
    (λ v → k (CPSVar v) t)
  ⟵⟨ rLet₁ (rLet₁ (rFun (λ x₁ → rFun (λ x₂ → rFun (λ x₃ →
           correctCont (pcontext-plug p₂ (Val (Var x₁)))
             ...
             {k₂ = λ v t'' → CPSIdk id₀ v (CPSCons ((proj₁ (aux₄ {var} (pcontext-plug p₂ (Val (Var x₁))) {c = c₁} (CPSIdk id₀) x₂ (CPSVar x₃)))) (CPSVar x₂) t'')}
             (λ v t₁ → sApp (sApp Subst≠ (sVal sVar=)) Subst≠) λ v t₁ → aux {μ[∙]μ₃ = μ[∙]μ₃} {μ[μα]μ₃ = μ[μα]μ₃} id₀ x₂ v (proj₁ (aux₄ {var} (pcontext-plug p₂ (Val (Var x₁))) {c = c₁} (CPSIdk id₀) x₂ (CPSVar x₃))) t₁))))) ⟩
             -}
  CPSLet
    (CPSLet
     (CPSVal
