...
ここで、環境が入った次の定理を提示する。定理\ref{HandMain}を使う事で補題\ref{KMove}を示すことが出来る。今後の節では定理\ref{HandMain}の証明、補題\ref{KMove}の証明の順に見ていく。
\begin{theorem}
...
\label{IdkContext}
\end{lemma}
これは該当のコンテキストがKの条件を満たす事を示しており、$\Idk$が登場するcontrolとpromptのケースで使う。\\
